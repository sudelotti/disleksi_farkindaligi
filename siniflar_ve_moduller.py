...
class VeriYoneticisi:
    ...
    def genel_analiz_ekle(self, ogr_no):
        # Bu fonksiyon sadece hem testler hem de anket bittiğinde çağrılır.
        dosya_ismi = f"{ogr_no}_detayli_rapor.txt"
        tarih = datetime.now().strftime("%d.%m.%Y %H:%M:%S")
        # Güncel verileri çek
        t = self.__veriler[ogr_no]["testler"]
        anket_puani = t.get("anket_puani", 0)
        ...
        metin = "\n" + "█"*60 + "\n"
        metin += f"GENEL DEĞERLENDİRME VE SONUÇ ({tarih})\n"
        metin += "█"*60 + "\n\n"
        
        metin += f"1. DİJİTAL TESTLER TOPLAM RİSK PUANI: {toplam_test_riski:.2f} / 11.00\n"
        metin += "-"*40 + "\n"
        metin += f"   - Fonoloji: {p1:.2f}\n"
        metin += f"   - Ses Farkındalığı: {p2:.2f}\n"
        metin += f"   - Sıralama: {p3:.2f}\n"
        metin += f"   - Heceleme: {p4:.2f}\n"
        metin += f"   - Hızlı Okuma: {p5:.2f}\n"
        metin += "-"*40 + "\n"
    ...
    def rapor_dosyasina_isles(self, ogr_no, test_adi, puan, hata_kayitlari, toplam_soru, yanlis_sayisi, okuma_suresi=None):
        ...
        if test_adi == "HIZLI OKUMA TESTİ":
            if ogr_no in self.__veriler:
                anket_p = self.__veriler[ogr_no]["testler"].get("anket_puani", 0)
                # Anket puanı 0'dan büyükse veya anket yapılmışsa (anket_puani > 0 varsayımı)
                # Not: Eğer anket sonucu 0 ise (mükemmel çocuk) bu mantık çalışmayabilir ama genelde anketten puan gelir.
                if anket_p > 0 or self.__veriler[ogr_no]["testler"].get("anket_puani") is not None:
                    self.genel_analiz_ekle(ogr_no)
    
    def anket_sonucu_isles(self, ogr_no, anket_puani):
        dosya_ismi = f"{ogr_no}_detayli_rapor.txt"
        tarih = datetime.now().strftime("%d.%m.%Y %H:%M:%S")

        # Sadece Anket Puanını Kaydet (Analiz Yok)
        metin = "\n" + "#"*60 + "\n"
        metin += f"ÖĞRETMEN GÖZLEM ANKETİ TAMAMLANDI ({tarih})\n"
        metin += f"PUAN: {anket_puani}\n"
        metin += "#"*60 + "\n\n"
        ...
        if ogr_no in self.__veriler:
             ...
             # Eğer veritabanında 'okuma_suresi' varsa ve 0'dan büyükse, öğrenci testi bitirmiştir.
             okuma_suresi = t.get("okuma_suresi", 0)
